...
namespace SkillersTest.DataSorter
{
    ...
    public class HugeFileSort
    {
        ...
        /// </summary>
        public Comparer<DataItem> Comparer { get; set; }
        ...
        {
            chunks = new SortedDictionary<string, ChunkInfo>(StringComparer.CurrentCulture);
        ...
        private void SortFile(string inputFileName, string outputFileName)
        {
            ...
            var entries = new List<DataItem>((int)(info.Length / 4L));
            using (var sr = new StreamReader(inputFileName, Encoding))
            {
                while (sr.Peek() >= 0)
                    entries.Add(DataItem.FromString(sr.ReadLine()));
            }
            Sorter.QuicksortParallel(entries, 0, entries.Count - 1, Comparer);
